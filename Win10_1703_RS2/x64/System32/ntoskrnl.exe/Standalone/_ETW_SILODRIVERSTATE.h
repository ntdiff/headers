...
typedef struct _ETW_SILODRIVERSTATE
{
  ...
  /* 0x0198 */ struct _EX_RUNDOWN_REF_CACHE_AWARE* EtwpLoggerRundown[64];
  /* 0x0398 */ struct _WMI_LOGGER_CONTEXT* WmipLoggerContext[64];
  /* 0x0598 */ struct _ETW_HASH_BUCKET EtwpGuidHashTable[64];
  /* 0x1398 */ unsigned short EtwpSecurityLoggers[8];
  /* 0x13a8 */ unsigned char EtwpSecurityProviderEnableMask;
  /* 0x13ac */ long EtwpShutdownInProgress;
  /* 0x13b0 */ unsigned long EtwpSecurityProviderPID;
  /* 0x13b8 */ struct _ETW_PRIV_HANDLE_DEMUX_TABLE PrivHandleDemuxTable;
  /* 0x13d8 */ struct _ETW_COUNTERS EtwpCounters;
  /* 0x13e8 */ union _LARGE_INTEGER LogfileBytesWritten;
  /* 0x13f0 */ struct _ETW_SILO_TRACING_BLOCK* ProcessorBlocks;
} ETW_SILODRIVERSTATE, *PETW_SILODRIVERSTATE; /* size: 0x13f8 */
