...
typedef struct _POP_FX_DEVICE
{
  ...
  /* 0x04b8 */ struct _WORK_QUEUE_ITEM ReportDevicePoweredOnPassiveWorkItem;
  /* 0x04d8 */ unsigned __int64 RelationsLock;
  /* 0x04e0 */ unsigned long IdleProviderCount;
  /* 0x04e8 */ struct _IRP* PendingDevicePowerIrp;
  /* 0x04f0 */ struct _LIST_ENTRY ExternalDependencies;
} POP_FX_DEVICE, *PPOP_FX_DEVICE; /* size: 0x0500 */
